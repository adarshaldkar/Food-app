...
export const getRestaurant = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const allRestaurants = await Restaurant.find({}).select('user restaurantName');
...
export const updateRestaurant = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (files && files.length > 0) {
            const imageUrl = await uploadImageOnCloudinary(files[0]);
    ...
    try {
        ...
        allOrders.forEach((order, index) => {
            console.log(`Order ${index + 1}:`, {
                ...
                resturant: (order as any).resturant, // Check if old field exists
                user: order.user,
                status: order.status
            });
        });
        ...
        const restaurantData = await Restaurant.findOne({ user: req.id });
        if (!restaurantData) {
        ...
        const orders = await Order.find({
            $or: [
                ...
                { resturant: restaurantData._id } as any
            ]
        })
...
export const  updateOrderStatus = async (req: Request, res: Response):Promise<any> => {
    try {
        ...
        const orderRestaurantId = orderDoc.restaurant || (orderDoc as any).resturant;
        console.log('Authorization check:', {
            orderRestaurantId,
            restaurantId: restaurant._id,
            hasRestaurant: !!orderDoc.restaurant,
            hasResturant: !!(orderDoc as any).resturant,
            orderDoc: { _id: orderDoc._id, restaurant: orderDoc.restaurant, resturant: (orderDoc as any).resturant }
        });
...
export const fixDuplicateRestaurants = async (req: Request, res: Response):Promise<any> => {
    try {
        ...
        for (const duplicate of duplicates) {
            ...
            restaurants.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
